...
MIN_NE_SIM = {'intra-lang':0.1, 'inter-lang':0.2} # for network inference
...
def read_and_filter_data(index_filename, min_art_ne_num):
    data = []
    print(datetime.datetime.now(), f"Reading data from {index_filename}")
    output_filename = ".".join(index_filename.split(".")[:-1]) + "_filtered_by_ne_count.index"
    with open(output_filename, "w") as out:
        with open(index_filename, "r") as fh:
            ...
            for line in fh:
                ...
                out.write(f"{story_id}\t{file}\t{lineno}\t{reladate}\t{vec}\n")
                n_lines += 1
                if n_lines%500000 == 0:
                    print(n_lines,"lines", end=" ", flush=True)

    print(datetime.datetime.now(), "Loaded", n_lines, "news articles from",index_filename)
...
def sort_indexes_by_count(index_filename, output_filename):
	named_entity_to_count = {}
	...
	with open(index_filename, "r") as fh:
		...
		for line in fh:
			...
			vec = tuple(ast.literal_eval(vec))
			for item in vec:
				if item[0] in named_entity_to_count.keys():
					named_entity_to_count[item[0]] += item[1]
				else:
					named_entity_to_count[item[0]] = item[1]

	sorted_named_entity_to_count = dict(sorted(named_entity_to_count.items(), key=lambda item: item[1], reverse=True))

	with open(output_filename, "w") as out:
		for key, val in sorted_named_entity_to_count.items():
			out.write(f'{key}\t{val}\n')
	print(f"Done writing sorted index counts to {output_filename}")
