...
// ./c_extract_pair <INPUT INDEX FILE> <OUTPUT FILENAME> <INDEX FILE WITH SORTED INDICES> <START DATE> <END DATE>

// ./c_extract_pair /work/pi_pgrabowicz_umass_edu/cbagchi/media_cloud_election_dashboard/data/mc_data_day_by_day/indexes/2024-01-01_to_2024-01-08-v5_ne-art-wiki-filtered_0_100000.index /work/pi_pgrabowicz_umass_edu/cbagchi/media_cloud_election_dashboard/data/mc_data_day_by_day/indexes/2024-01-01_to_2024-01-08-v5_filtered_only_extracted_pairs.txt /work/pi_pgrabowicz_umass_edu/cbagchi/media_cloud_election_dashboard/data/mc_data_day_by_day/indexes/2024-01-01_to_2024-01-08-v5sorted_by_count.index 0 100000
...
std::vector<int> read_file_get_top_k_ne(const std::string& fileName, int k) {
    ...
    while (std::getline(file, line)) {
        ...
        if (iss >> firstPart) {
            try {
                ...
                std::cerr << "Warning: Non-integer value found in the first part of the line: " << firstPart << std::endl;
            }
        }
...
int main(int argc,char **argv) {
    ...
    //  int top_ne[50] = {408352, 148, 84263196, 28775762, 794, 25337, 668, 127, 18706971, 129, 130, 60, 105, 38, 128, 11746, 2570643, 132, 846570, 131, 30, 219311, 183, 29, 421211, 159, 228905, 2495862, 99, 89469904, 16, 3947, 25274, 46, 61, 463180, 96, 228563, 408, 142, 130879, 226494, 17, 956, 812, 23806, 7188, 43, 224475, 1033};
     vector<int> top_ne = read_file_get_top_k_ne(argv[3], stoi(argv[4])); // Hard-coded Top NE (=50)
    //  int top_ne[50] = {408352, 148, 84263196, 28775762, 794, 25337, 668, 127, 18706971, 129, 130, 60, 105, 38, 128, 11746, 2570643, 132, 846570, 131, 30, 219311, 183, 29, 421211, 159, 228905, 2495862, 99, 89469904, 16, 3947, 25274, 46, 61, 463180, 96, 228563, 408, 142, 130879, 226494, 17, 956, 812, 23806, 7188, 43, 224475, 1033};
    ...
    for(int ne : top_ne)
     ...
     int date_diff = 7; //Check over a week
    //  int start_date = stoi(argv[4]);
    //  int end_date = stoi(argv[5]);
    ...
    //  infile.open("/work/pi_pgrabowicz_umass_edu/cbagchi/media_cloud_election_dashboard/data/mc_data_day_by_day/indexes/2024-01-01_to_2024-01-08-v5_ne-art-wiki-filtered_0_100000.index", ios::in);
     ...
     while (getline(infile, buf)) {
         ...
         if (line_num % 2 == 0){
            ...
            // for (int i = 0; i < cur_group_size; ++i){
                // cout<<dat_list[i]<<" "<<ind_list[i]<<" "<<ind_line_list[i]<<" "<<ind_line_string_list[i]<<endl;
            // }
             // extract the pairs
    ...
    //  outfile.open("/work/pi_pgrabowicz_umass_edu/cbagchi/media_cloud_election_dashboard/data/mc_data_day_by_day/indexes/2024-01-01_to_2024-01-08-v5_filtered_only_extracted_pairs_" + to_string(start_date) + "_" + to_string(end_date) + ".txt", ios::out);
