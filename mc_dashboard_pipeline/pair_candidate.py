...
'''
python pair_candidate.py -i /work/pi_pgrabowicz_umass_edu/cbagchi/media_cloud_election_dashboard/data/mc_data_day_by_day/indexes/2024-01-01_to_2024-01-08-v5_filtered_only_extracted_pairs_0_100000.txt -ind pp -s 0 -e 10000
'''
## Hard coded index file
...
def load_article(file,lineno):
	...
	# print(file.replace(".json", ".offsets"))
	with open(file.replace(".json", ".offsets")) as fh:
...
def read_data(index_filename):
	...
	with open(index_filename, "r") as fh:
		...
		print("Loaded...", end=" ")
		for line in fh:
			story_id, file, lineno, reladate, vec = line.strip().split("\t")
...
# rank starts at 0
def find_pair_candidates(input_file, start_date, end_date, outdir, jaccard_sim_intra_lang, jaccard_sim_inter_lang):
	...
	# data_list.append(read_data(f"/work/pi_pgrabowicz_umass_edu/cbagchi/media_cloud_election_dashboard/data/mc_data_day_by_day/indexes/2024-01-01_to_2024-01-08-v5_filtered.index"))
	index_file = os.path.join("/".join(input_file.split("/")[:-1]), "indexes_filtered_by_ne_count.index")
	data_list.append(read_data(index_file))
	# data_list.append(read_data(f""))

	# outdir = f"/work/pi_pgrabowicz_umass_edu/cbagchi/media_cloud_election_dashboard/data/mc_data_day_by_day/candidates/candidates-top10-ne-filtered_{start_date}_{end_date}"
	if not os.path.exists(outdir):
		os.makedirs(outdir)
	print("Will save results to directory:", outdir, flush=True)
	# filename = f"{outdir}/{datetime.datetime.now():%Y-%m-%d_%H:%M:%S}.jsonl"
	filename = f'{outdir}/final_candidates.jsonl'
	...
	with open(filename, "w") as output:
		with open(input_file, "r") as fh:
			for line in fh:
				...
				if (jaccard >= jaccard_sim_intra_lang and input_type=='intra-lang') or (jaccard >= jaccard_sim_inter_lang and input_type=='inter-lang'):
					...
					if input_type == 'intra-lang':
						...
						# print(art1.file)
						a1 = load_article(art1.file, art1.lineno)
						# print(a1)
						...
						a2 = load_article(art2.file, art2.lineno)
						text1 = a1["text"].strip()
						text2 = a2["text"].strip()
...
if __name__ == '__main__':
	...
	parser.add_argument("--jaccard_sim_intra_lang", dest="jaccard_sim_intra_lang",
						required=True, type=float,
						help="Jaccard similarity for intra language pairs")
	parser.add_argument("--jaccard_sim_inter_lang", dest="jaccard_sim_inter_lang",
						required=True, type=float,
						help="Jaccard similarity for inter language pairs")
	args = parser.parse_args()

	'''loading the pairs, continue here'''
	find_pair_candidates(args.input_file, args.start_date, args.end_date, args.outdir, args.jaccard_sim_intra_lang, args.jaccard_sim_inter_lang)
